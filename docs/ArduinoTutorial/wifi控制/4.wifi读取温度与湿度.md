...
## 2.2.5.4 ESP-01S 代码说明

打开`ESP01_S代码`中的代码文件，将不用显示的按键以及传感器数据显示注释掉，将温度与湿度数据显示取消注释即可（你也可以使用快捷键`Ctrl + /`，鼠标光标点击到需要注释的代码行或选中的代码多行代码按`Ctrl + /`即可达到注释或者取消注释的效果 ）
...
① 前面的基础框架wifi读取电位器值的代码一样，区别就在后面的读取数据代码上

② 使用`if`判断是否接收到ESP-01S模块发送的字符`SENSOR_READ`，如果是就使用串口打印函数发送数据给ESP-01S模块

③ 发送方式假如需要发送温度值25给ESP-01S模块显示到网页上发送格式如下:`Serial.println("TEMP" + String(25))`，湿度也是以同样方式发送的
